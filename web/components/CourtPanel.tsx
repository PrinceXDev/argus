...
export function CourtPanel({ ruling }: { ruling: Ruling }) {
	...
	if (ruling.disputes.length === 0 && ruling.corroboration.length === 0)
		return null;
	...
	return (
		<div className="rounded-lg border border-line bg-panel p-4">
			...
			{bottlenecked.map((c) => (
				...
				>
					<span className="font-medium text-contested">
						Corroboration is not independent.
					</span>{" "}
					<span className="text-fg/85">{c.note}</span>
					<span className="mt-1.5 block font-mono text-[11px] text-muted">
						{c.directSources} sources · {c.independentFlow} units of independent
						support · {c.originCount} origin{c.originCount === 1 ? "" : "s"}
					</span>
				</p>
			))}
